...
class MTGBiasDetector:
    ...
    def __init__(self, random_state: int = None, n_bootstrap: int = 1000):
        ...
        # Define representation groups based on Scryfall Tagger tags
    ...
    def analyze_representation_bias(self, df: pd.DataFrame,
        ...
        for group_name in self.representation_groups.keys():
            ...
            # Calculate disparate impact ratio (hiring bias metric adapted)
            ...
            result = CardBiasResult(
                ...
                group_stats={
                    ...
                    'effect_size': (group_mean - control_mean) / np.sqrt(((group_values.std()**2 + control_values.std()**2) / 2))
                },
            ...
            print(f"  Effect size: {result.group_stats['effect_size']:.3f}")
...
def main():
    ...
    output_data = {
        ...
        'dataset_summary': {
            'total_cards': len(df),
            'cards_by_group': {group: df[f'is_{group}'].sum()
