"""FormatTransformer skill class wrapper for A2A orchestration."""
...
class FormatTransformerSkill(Skill):
    """
    Skill wrapper for converting files to PDF.

    Wraps .claude/skills/pdf/convert-to-pdf.py subprocess execution.
    """

    def __init__(self, skill_name: str = "format-transformer"):
        """Initialize FormatTransformer skill."""
        super().__init__(skill_name, "Convert files to PDF using LibreOffice")
        self.script_path = Path(__file__).resolve().parents[2] / ".claude" / "skills" / "pdf" / "convert-to-pdf.py"

    async def process_request(self, request_message: A2AMessage) -> A2AMessage:
        """
        Process conversion request by invoking subprocess.

        Args:
            request_message: A2A REQUEST message with payload:
                - source_path: Path to source file (PPTX)
        ...
        try:
            ...
            try:
                stdout, stderr = await asyncio.wait_for(
                    process.communicate(),
                    timeout=120.0  # 2 minutes for LibreOffice PDF conversion
                )
    ...
    def _build_command(self, payload: Dict[str, Any]) -> list[str]:
        ...
        Args:
            payload: Request payload with source_path, output_dir
        ...
        command = [
            ...
            "--output-dir", payload.get("output_dir", "outputs")
        ]
